...
alleleSetup.call_setup_alleles <- function( currentSample, uniqueSamDT, setup.knownSnpDFList, hetRatio, setup.minDP, ambScore=T, allPosScore=F, onlyExonScore=F, homScoreBuffer=4, includeAmb=F, addDiversity=F, combineTypings=F, skipSnpGen=F, skipSet=F){
  ...
  if( 'KIR2DL1' %in% names(nonAmbAlleleCall.list) ){
    pos4710 <- as.integer( currentSample$kffHits[['*KIR2DL1*4710b']] ) > refine.probeThresh
    pos47 <- as.integer( currentSample$kffHits[['*KIR2DL1*4and7']] ) > refine.probeThresh
    pos7 <- as.integer( currentSample$kffHits[['*KIR2DL1*7only']] ) > refine.probeThresh
    pos003 <- as.integer( currentSample$kffHits[['*KIR2DL1*003']] ) > refine.probeThresh
  ...
  if( 'KIR2DL2' %in% names(nonAmbAlleleCall.list) ){
    posE5292T <- as.integer( currentSample$kffHits[['*KIR2DL2*E5292T']] ) > refine.probeThresh
    pos00103 <- as.integer( currentSample$kffHits[['*KIR2DL2*00103']] ) > refine.probeThresh
    pos004 <- as.integer( currentSample$kffHits[['*KIR2DL2*004']] ) > refine.probeThresh
  ...
  if( 'KIR2DL4' %in% names(nonAmbAlleleCall.list) ){
    pos10A <- as.integer( currentSample$kffHits[['*KIR2DL4*10A64']] ) > refine.probeThresh
    pos9A <- as.integer( currentSample$kffHits[['*KIR2DL4*9A64']] ) > refine.probeThresh
  ...
  if( 'KIR2DS1' %in% names(nonAmbAlleleCall.list) ){
    pos001 <- as.integer( currentSample$kffHits[['*KIR2DS1*001']] ) > refine.probeThresh
    pos00202.00302 <- as.integer( currentSample$kffHits[['*KIR2DS1*00202^00302']] ) > refine.probeThresh
    pos005 <- as.integer(currentSample$kffHits[['*KIR2DS1*005']]) > refine.probeThresh
    pos003 <- as.integer(currentSample$kffHits[['*KIR2DS1*00301']]) > refine.probeThresh
    pos006 <- as.integer(currentSample$kffHits[['*KIR2DS1*006']]) > refine.probeThresh
    pos004.008 <- as.integer(currentSample$kffHits[['*KIR2DS1*004^008']]) > refine.probeThresh
    pos009 <- as.integer(currentSample$kffHits[['*KIR2DS1*009']]) > refine.probeThresh
  ...
  if( 'KIR2DS4' %in% names(nonAmbAlleleCall.list) ){
    pos003 <- as.integer(currentSample$kffHits[['*KIR2DS4*del003']]) > 10
    pos006 <- as.integer(currentSample$kffHits[['*KIR2DS4*del006']]) > 10
    pos009 <- as.integer(currentSample$kffHits[['*KIR2DS4*009']]) > 10
  ...
  if( 'KIR3DP1' %in% names(nonAmbAlleleCall.list) ){
    ...
    pos3DP1del <- as.integer( currentSample$kffHits[['*KIR3DP1*E2del']] ) > refine.probeThresh
    neg3DP1del <- as.integer( currentSample$kffHits[['*KIR3DP1*E2nodel']] ) > refine.probeThresh
  ...
  if( 'KIR3DS1' %in% names(nonAmbAlleleCall.list) ){
    pos049 <- as.integer( currentSample$kffHits[['*KIR3DS1*049']] ) > refine.probeThresh
...
ping_allele <- function(sampleList){
  ...
  for( currentSample in sampleList ){
    ...
    if( !currentSample$failed ){
      uniqueSamDT <- alleleSetup.process_samDT( currentSample$iterSamPathList[[1]], delIndex.list, processSharedReads = setup.readBoost, readBoost.thresh )}
